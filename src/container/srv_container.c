...
#define EC_AGG_EPH_INTV	 (10ULL * NSEC_PER_SEC)	/* seconds interval to check*/
static void
cont_agg_eph_leader_ult(void *arg)
{
	struct dss_sleep_ult	*sleep_ult;
	struct cont_svc		*svc = arg;
	int			rc = 0;

	sleep_ult = dss_sleep_ult_create();
	if (sleep_ult == NULL) {
		D_ERROR(DF_UUID " sleep ult creation failed.\n",
			DP_UUID(svc->cs_pool->sp_uuid));
		return;
	}
	...
	while (1) {
		...
		rc = map_ranks_init(svc->cs_pool->sp_map, MAP_RANKS_DOWN,
				    &fail_rank_list);
		if (rc) {
			D_ERROR(DF_UUID": ranks init failed: %d\n",
				DP_UUID(svc->cs_pool->sp_uuid), rc);
			goto sleep;
		}
		...
		d_list_for_each_entry(ec_agg, &svc->cs_ec_agg_list,
			...
			if (svc->cs_pool->sp_stopping)
				break;

			for (i = 0; i < ec_agg->ea_servers_num; i++) {
				if (d_rank_in_rank_list(&fail_rank_list,
				    ec_agg->ea_server_ephs[i].rank)) {
			...
			rc = cont_iv_ec_agg_eph_refresh(
						svc->cs_pool->sp_iv_ns,
						ec_agg->ea_cont_uuid, min_eph);
			if (rc) {
				D_ERROR(DF_CONT": refresh failed: %d\n",
					DP_CONT(svc->cs_pool->sp_uuid,
		...
		map_ranks_fini(&fail_rank_list);
		if (svc->cs_ec_agg_stopping)
			break;
sleep:
		dss_ult_sleep(sleep_ult, EC_AGG_EPH_INTV);
	}
	...
	ABT_mutex_lock(svc->cs_ec_agg_stop_mutex);
	ABT_cond_signal(svc->cs_ec_agg_stop_cond);
	ABT_mutex_unlock(svc->cs_ec_agg_stop_mutex);
}

static void
cont_svc_ec_agg_destory(struct cont_svc *svc)
{
	struct cont_ec_agg *ec_agg;
	...
	if (svc->cs_ec_agg_stop_cond)
		ABT_cond_free(&svc->cs_ec_agg_stop_cond);

	if (svc->cs_ec_agg_stop_mutex)
		ABT_mutex_free(&svc->cs_ec_agg_stop_mutex);
}
...
cont_svc_ec_agg_leader_start(struct cont_svc *svc)
{
	...
	rc = ABT_cond_create(&svc->cs_ec_agg_stop_cond);
	if (rc != ABT_SUCCESS)
		D_GOTO(out, rc = dss_abterr2der(rc));

	rc = ABT_mutex_create(&svc->cs_ec_agg_stop_mutex);
	if (rc != ABT_SUCCESS)
		D_GOTO(out, rc = dss_abterr2der(rc));

	rc = dss_ult_create(cont_agg_eph_leader_ult, svc, DSS_ULT_MISC,
			    DSS_TGT_SELF, 0, NULL);
	if (rc)
...
{
	svc->cs_ec_agg_stopping = 1;

	ABT_mutex_lock(svc->cs_ec_agg_stop_mutex);
	ABT_cond_wait(svc->cs_ec_agg_stop_cond,
		      svc->cs_ec_agg_stop_mutex);
	ABT_mutex_unlock(svc->cs_ec_agg_stop_mutex);
	cont_svc_ec_agg_destory(svc);
}
