		...
		mode_t *mode, struct stat *stbuf)
{
	...
	/* Paths from dfs->root must be absolute.
	 * Paths from others must be relative. */
	if (root == &dfs->root) {
		if (path[0] != '/')
			return ENOENT;
	} else {
...
lookup_rel_path_loop:
		...
		//D_ERROR("==== lookup_loop: obj closed\n");

		if (!exists)
			D_GOTO(err_obj, rc = ENOENT);
		//D_ERROR("==== lookup_loop: exists\n");
		...
		if (S_ISLNK(entry.mode)) {
			...
			if (!(flags & O_NOFOLLOW)) {
				//D_ERROR("==== recurse 2: %s / %s\n", parent.name, entry.value);

				if (!parent_fully_valid &&
				    strncmp(entry.value, "..", 2) == 0) {
					//D_ERROR("==== recurse 2: ENOTSUP\n");
					D_FREE(entry.value);
					D_GOTO(err_obj, rc = ENOTSUP);
				}
...
dfs_chmod(dfs_t *dfs, dfs_obj_t *parent, const char *name, mode_t mode)
{
	...
	if (S_ISLNK(entry.mode)) {
		...
		rc = lookup_rel_path(dfs, parent, entry.value, O_RDWR, &sym, NULL, NULL);
		if (rc) {
